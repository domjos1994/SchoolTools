 ...
 */
package de.domjos.schooltoolslib.utils;
 ...
 * @see de.domjos.schooltoolslib.marklist.de.GermanLinearList
 * @see de.domjos.schooltoolslib.marklist.de.GermanListWithCrease
 * @author Dominic Joas
 * @version 1.0
 */
public class MathUtils {
    ...
    /**
     * Function which calculates a line between two points
    ...
    public static Map<Double, Double> calculateLineBetweenTwoPoints(double x1, double y1, double x2, double y2, double xMultiplier, double yMultiplier) {
        return MathUtils.calculateLineBetweenTwoPoints(x1, y1, x2, y2, xMultiplier, yMultiplier, new LinkedHashMap<>());
    }
    ...
    /**
     * Function which calculates a exponential crease to points
     * and adds the rounded values to an available map
     * @param x1 X-Value (Point 1)
     * @param y1 Y-Value (Point 1)
     * @param x2 X-Value (Point 2)
     ...
     * @return the Map with the Values
     */
    public static Map<Double, Double> calculateExponentialCreaseBetweenTwoPoints(double x1, double y1, double x2, double y2, double xMultiplier, double yMultiplier, Map<Double, Double> values) {
        int i = 0;
        if(x1<x2) {
            double numberOfCycles = (x2 - x1) / xMultiplier;
            for(double curX = x1; curX <= x2; curX += xMultiplier, i++) {
                if(y1<y2) {
                    double val = Math.pow((y2 - y1), 1/numberOfCycles);
                    values.put(MathUtils.round(curX, xMultiplier), (1+y2) - MathUtils.round(Math.pow(val, i), yMultiplier));
                } else {
                    double val = Math.pow((y1 - y2), 1/numberOfCycles);
                    values.put(MathUtils.round(curX, xMultiplier), (1+y1) - MathUtils.round(Math.pow(val, i), yMultiplier));
                }
            ...
            double numberOfCycles = (x1 - x2) / xMultiplier;
            for(double curX = x1; curX >= x2; curX -= xMultiplier, i++) {
                if(y1<y2) {
                    double val = Math.pow((y2 - y1), 1/numberOfCycles);
                    values.put(MathUtils.round(curX, xMultiplier), MathUtils.round(Math.pow(val, i), yMultiplier));
                } else {
                    double val = Math.pow((y1 - y2), 1/numberOfCycles);
                    values.put(MathUtils.round(curX, xMultiplier), MathUtils.round(Math.pow(val, i), yMultiplier));
                }
    ...
    public static double round(double value, double multiplier) {
        ...
        if(strValue.endsWith("5")) {
            ...
            value = Double.parseDouble(digitsToPoint + Integer.parseInt(digitsFive));
