...
public class PDFBuilder {
    ...
    public PDFBuilder(String path, byte[] icon, Context context) throws Exception {
        ...
        writer.setPageEvent(new Footer(context, icon));
        this.document.open();
    ...
    public static void openPDFFile(String path, Context context) throws Exception {
...
class Footer extends PdfPageEventHelper {
    ...
    Footer(Context context, byte[] icon) {
    ...
    public void onEndPage(PdfWriter writer,Document document) {
        try {
            ...
            Image img = Image.getInstance(this.icon);
            ...
            ColumnText.showTextAligned(writer.getDirectContent(),Element.ALIGN_CENTER, new Phrase(String.valueOf(document.getPageNumber())), rect.getRight(), rect.getBottom(), 0);
        } catch (Exception ex) {
